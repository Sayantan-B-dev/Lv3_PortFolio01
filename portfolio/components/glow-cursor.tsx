...
export function GlowCursor({
  ...
  useEffect(() => {
    // fullscreen overlay only makes sense with a fine pointer
    ...
    const updatePointer = (clientX: number, clientY: number) => {
      ...
      const y = clamp(height - clientY, 0, height);
      if (!initialized) initializeTrail(clientX, clientY);
      else {
        target.x = x;
        target.y = y;
      }
    ...
    const onTouchMove = (event: TouchEvent) => {
      if (event.touches.length > 0) updatePointer(event.touches[0].clientX, event.touches[0].clientY);
    };
    ...
    const render = (now: number) => {
      ...
      if (initialized) {
        // stiff follow: head snaps fast, chain stays rigid (minimal snake delay)
        const headEase = 1 - Math.pow(1 - clamp(config.followSpeed, 0.01, 0.99), delta);
        const chainBase = clamp(0.55 + config.followSpeed * 0.4, 0.08, 0.97);
  ...
  return (
    <div
      ref={containerRef}
      className={className ? `glow-cursor ${className}` : "glow-cursor"}
      style={style}
      {...rest}
    >
